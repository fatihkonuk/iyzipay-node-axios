...
IyzipayResource.prototype._request = function (method, cb) {
    ...
    const options = {
        ...
        params: this._getQueryString(method), // Query string for GET requests
        data: this._getBody(method) // Body data for POST requests
    };
    axios(options)
        .then(response => {
            // Axios returns response.data for the body by default
        ...
        .catch(error => {
            ...
            if (error.response) {
                ...
                cb(new Error(`Server responded with status ${error.response.status}: ${error.response.data}`));
            } else if (error.request) {
                // The request was made but no response was received
                cb(new Error('No response received from the server'));
            } else {
                // Something happened in setting up the request that triggered an Error
                cb(new Error('Error setting up the request'));
            }
        });
};
